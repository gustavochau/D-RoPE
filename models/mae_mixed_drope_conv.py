...
from models.mixedtransformerblock_drope import TransformerBlock
...
class MaskedAutoEncoderViT(nn.Module):
    ...
    def __init__(
        ...
        gamma_distance: float = 0.5,
        beta: float = 0.3,
        num_dec_conv_layers: int = 4,
        ...
        self.num_dec_conv_layers = num_dec_conv_layers
        self.conv_kernel_size = conv_kernel_size
        self.decoder_conv = self.conv_block(1, 1, self.num_dec_conv_layers, kernel_size=self.conv_kernel_size)
        self.mask_to_apply = 0 # control which type of mask for alternate
        self._init_weights()

    def _init_weights(self):
        """Initialize weights for positional embeddings, mask tokens, and cls tokens.
        ...
        self.apply(self.conv_init_weights)

    def conv_init_weights(self, m):
        """Initialize convolutional layer weights using Kaiming normalization."""
        if isinstance(m, nn.Conv2d):
            nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
            if m.bias is not None:
                nn.init.constant_(m.bias, 0)

    def angle_between_vectors_spherical(self, theta1, phi1, theta2, phi2):
        """Compute angle between two unit vectors given in spherical coordinates (theta, phi)."""
        cos_gamma = (torch.sin(theta1) * torch.sin(theta2) * torch.cos(phi1 - phi2) +
                    torch.cos(theta1) * torch.cos(theta2))
        return torch.arccos(torch.abs(torch.clip(cos_gamma, -1.0, 1.0)))

    def compute_distances(self, posq, posk):
        """Compute pairwise customized distance between positions in spherical coordinates (rho, theta, phi)."""
        rho_q = posq[...,0].unsqueeze(1)
        theta_q = posq[...,1].unsqueeze(1)
        phi_q = posq[...,2].unsqueeze(1)
        rho_k = posk[...,0].unsqueeze(-1)
        theta_k = posk[...,1].unsqueeze(-1)
        phi_k = posk[...,2].unsqueeze(-1)
        d = torch.sqrt(self.gamma_distance*(rho_q-rho_k)**2 + self.angle_between_vectors_spherical(theta_q,phi_q,theta_k,phi_k)**2)
        return d
    ...
    def conv_block(self, in_channels, out_channels, n_layers, kernel_size=3):
        """Build a 3D convolutional block with multiple layers and ReLU activations."""
        padding = kernel_size // 2
        layers = []
        n_inner = n_layers-1
        out_num = [8 for x in range(n_inner)]+[1]
        for i, _ in enumerate(range(n_layers)):
            layers.append(nn.Conv3d(in_channels, out_num[i], kernel_size=kernel_size, stride=1, padding=padding))
            layers.append(nn.ReLU(inplace=True))
            in_channels = out_num[i]
        return nn.Sequential(*layers)

    def gen_arguments(self, dists):
        """Generate D-RoPE arguments from distance matrix."""
        # dists is h x h (assumed to be a torch tensor already)
        device = dists.device  # Make sure we stay on the same device
        d = self.decoder_hidden_size//self.num_heads
        comp = dists.unsqueeze(-1).expand(-1, -1, -1, d // 2)  # (w, h, d/2)
        beta = 0.3
        n = torch.arange(0, d // 2, device=device)
        Theta = beta ** (-2 * n / d)  # (d/2,)
        Theta = Theta.view(1, 1, -1)  # (1,1,d/2)
        comp = comp * Theta  # element-wise multiply
        comp = comp.repeat(1, 1, 1, 2)  # Repeat along last dimension

        return comp

    def _masking(self, x, spherical, masking_ratio: float | None = None):
        ...
        spherical_masked = spherical[torch.arange(batch_size).unsqueeze(1), selected_indices]
        ...
        return x_masked, selected_indices, mask, spherical_masked

    def _diff_masking(self, x, spherical, masking_ratio: float | None = None):
    ...
    def forward_mask(self,x, spherical, t, seed = None):
        ...
        if self.mask_to_apply == 1: #diffusion masking
            ...
            spherical = rearrange(spherical,'(b t) s d -> b t s d', t=t)
            x, selected_indices, mask, spherical_masked = self._diff_masking(x, spherical, masking_ratio=self.mask_ratio_diff)
        elif self.mask_to_apply == 0: #space masking
            x, selected_indices, mask, spherical_masked = self._masking(x, spherical, masking_ratio=self.mask_ratio_space)
            x = rearrange(x,'(b t) s d -> b t s d', t=t)
            spherical_masked = rearrange(spherical_masked,'(b t) s d -> b t s d', t=t)
        B, T, S, D = x.shape
        return x, selected_indices, mask, spherical_masked, B, T, S ,D 

    def forward_encode(self,x,b,t,s,components_rope):
        """Pass tokens through encoder transformer blocks with D-RoPE."""
        x = rearrange(x,'b t s d -> b (s t) d')
        cls_tokens = self.cls_token.expand(x.size(0), -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
        # encoder transformer blocks
        for blk in self.blocks:
            x = blk(x, b,t,s, components_rope)
        x = self.ln(x)
        return x
    ...
    def forward_decode(self, x_, spherical, positions, cls_token, b, t, s, components_rope):
    ...
    def forward_nomask(self,x,spherical, positions):
        ...
        spherical_expanded = repeat(spherical, 'b t d -> b t s d', s=s)
        spherical_expanded = rearrange(spherical_expanded, 'b t s d -> (b s) t d')
        dist_t = self.compute_distances(spherical_expanded, spherical_expanded)
        components_rope = self.gen_arguments(dist_t)
        x = rearrange(x,'(b t) s d -> b t s d', t=t)
        x = self.forward_encode(x,b,t,s,components_rope)
    ...
    def forward(self, x, spherical, positions, seed = None):
        ...
        x, b, t, s, d = self.forward_patch_embed(x,spherical, positions)
        spherical_expanded = repeat(spherical, 'b t d -> b t s d', s=s)
        spherical_expanded = rearrange(spherical_expanded, 'b t s d -> (b t) s d')

        # Masking
        x, selected_indices, mask, spherical_masked, B, T, S ,D = self.forward_mask(x,spherical_expanded,t,seed)
        spherical_masked = rearrange(spherical_masked, 'b t s d -> (b s) t d')

        # Generate components for RoPE
        dist_t = self.compute_distances(spherical_masked, spherical_masked)
        ...
        dist_t = self.compute_distances(spherical_expanded, spherical_expanded)
        components_rope = self.gen_arguments(dist_t)
        x = self.forward_decode(x_,spherical,positions, cls_token,b,t,s,components_rope)
     
        cls = x[:, 0, :]
        x = x[:,1:,:]
        x = rearrange(x,'b (s t) d -> b t s d', t=t)
        x = self.unpatchify(x)
        x = rearrange(x,'b h w z t -> (b t) 1 h w z', t=t)
        x = self.decoder_conv(x)
        x = rearrange(x,'(b t) 1 h w z -> b h w z t ', t=t)
        x = self.patchify(x)
