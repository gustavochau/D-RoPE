...
from utilities.data_utils_downstream import DiffMRIDataset_adni, CollateConcat
...
_NAME_PROJ = f"{_TASK}_maerope_adnimci_LoRA_conv"
_NUM_TRAIN_LAYERS = 1
...
_NUM_POS = 0.29
_NUM_NEG = 0.71
_POS_WEIGHT = torch.tensor([_NUM_NEG / _NUM_POS])  # = 9.0
...
def train(config, split_num=0):
    ...
    # Create the model with the backbone and the head, and move it to the appropriate devices
    model = ViTWithHead(
        backbone,
        emb_size=emb_size,
    ...
    criterion = nn.BCEWithLogitsLoss(pos_weight=_POS_WEIGHT.to(device_backbone))
    ...
    for epoch in range(max_epochs):
        ...
        if (epoch+1)%_INTERVAL_RECON == 0:
            ...
            metrics["epoch"]=epoch
            wandb.log(metrics)  
            csv_out = os.path.join(wandb.run.dir,f"result-tables-{_TASK}-val-mci_{epoch+1}.cvs")
            ...
            if cur_val > best_val:
                ...
                csv_out = os.path.join(wandb.run.dir,f"result-tables-{_TASK}-test-mci.cvs")
                df_all_test.to_csv(csv_out, index=False) 

    metrics, df_all = evaluate_metrics(model,val_loader)   
    wandb.log(metrics)  
    csv_out = os.path.join(wandb.run.dir,f"result-tables-{_TASK}-val-mci.cvs")
    df_all.to_csv(csv_out, index=False) 
...
def train_epoch(model, train_loader, optimizer, criterion):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        __, x, positions, rho, theta, phi, group_label, age = data
        spherical = reformat_data(rho, theta, phi)
        optimizer.zero_grad()
        with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16):

            out = model.forward(x,spherical,positions)
            loss = criterion(out,group_label.to(device_backbone))
    ...
    try:
        del x, positions, rho, theta, phi, group_label, spherical, out, loss
    except Exception:
        pass
...
def evaluate_epoch_loss(model, val_loader, criterion):
    ...
    with torch.no_grad():
        for i, data in enumerate(val_loader, 0):
            ...
            __, x, positions, rho, theta, phi, group_label, __ = data
            spherical = reformat_data(rho, theta, phi)
...
def evaluate_metrics(model, metric_loader):
    ...
    with torch.no_grad():
        for i, data in enumerate(metric_loader, 0):
            ...
            all_labels.extend(group_label.numpy().flatten())
            all_ids.extend(ids)

        try:
            del x, positions, rho, theta, phi, group_label, spherical, logits
        except Exception:
            pass
        torch.cuda.empty_cache()
        gc.collect()
