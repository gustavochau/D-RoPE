...
from utilities.data_utils_downstream import DiffMRIDataset_agesex, CollateConcat
...
_NAME_PROJ = f"{_TASK}_maerope_agepred_LORA_conv"
_NUM_TRAIN_LAYERS = 1
...
parameters_dict = {
    ...
    'finetune_strat':{
        'value': 1  # 0: linear probe 1: fine-tune
    },
    'batch_size': {
        'value': 6
        },
...
class ViTWithHead(nn.Module):
    ...
    def forward(self, x, spherical, positions):
        # -- Change: Ensure inputs are on the backbone's device (GPU 0)
...
def train(config, split_num=0):
    ...
    proj_name = f"{_NAME_PROJ}_cv_{split_num}"
    wandb.init(project=proj_name, config=config)

    # Load split lists using the requested split number. Filenames are expected
    # to follow the pattern: '<split>_subject_list_agdev_{split_num}'
    splits = ['train', 'val', 'test']
    list_ids = {}
    for split in splits:
        split_file = os.path.join(_SPLIT_FOLDER, f"{split}_subject_list_agdev_{split_num}")
    ...
    dataset_train = DiffMRIDataset_agesex(folder_path=_DATASET_FOLDER, task = _TASK, ids=list_ids['train'], 
                                    format_diff = 'spherical', rand_sel=0, rand_dir=_RANDOM_DIR, predef_y=0)
    train_loader = DataLoader(dataset_train, batch_size=batch_size, shuffle=True, 
                                    collate_fn=CollateConcat()) 
    dataset_val = DiffMRIDataset_agesex(folder_path=_DATASET_FOLDER, task = _TASK, ids=list_ids['val'], 
                                    format_diff = 'spherical', rand_sel=0, rand_dir=_RANDOM_DIR, predef_y=0)
    val_loader = DataLoader(dataset_val, batch_size=batch_size, shuffle=True, collate_fn=CollateConcat())

    dataset_test = DiffMRIDataset_agesex(folder_path=_DATASET_FOLDER, task = _TASK, ids=list_ids['test'], 
                                    format_diff = 'spherical', rand_sel=0, rand_dir=_RANDOM_DIR, predef_y=0)
    test_loader = DataLoader(dataset_test, batch_size=batch_size, shuffle=True, collate_fn=CollateConcat())
    ...
    backbone = backbone.to(torch.float) #bf16

    # Load weights
    backbone.load_state_dict(torch.load(weights_path, weights_only=True))

    #backbone.patch_size = (8,8,8)

    # -- Change: Freeze all parameters of the backbone BEFORE applying LoRA
    ...
    # get_peft_model makes the new LoRA parameters trainable
    # -- Change: Instantiate the model with device assignments
    # The backbone (with its frozen and LoRA parts) and the head will be
    # moved to their respective devices inside the model's __init__ method.
    model = ViTWithHead(
        backbone,
        emb_size=emb_size,
    ...
    # The PEFT utility will print parameters that are trainable.
    # You will see that only the LoRA modules are trainable.
    #model.backbone.print_trainable_parameters()
    model.lastblock.print_trainable_parameters()
    #count_params(model.lastblock)
    ...
    for epoch in range(max_epochs):
        ...
        if (epoch+1)%_INTERVAL_RECON == 0:
            ...
            metrics["epoch"]=epoch
            wandb.log(metrics)  
            csv_out = os.path.join(wandb.run.dir,f"result-tables-{_TASK}-val-age_{epoch+1}.cvs")
            ...
            if cur_val > best_val:
                ...
                # Save test evaluation in the model with the best validation performance
                ...
                csv_out = os.path.join(wandb.run.dir,f"result-tables-{_TASK}-test-age.cvs")
                df_all_test.to_csv(csv_out, index=False) 
    metrics, df_all = evaluate_metrics(model,val_loader)   
    wandb.log(metrics)  
    csv_out = os.path.join(wandb.run.dir,f"result-tables-{_TASK}-val-age.cvs")
    df_all.to_csv(csv_out, index=False) 
...
def evaluate_metrics(model, metric_loader):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(metric_loader, 0):
            ids, x, positions, rho, theta, phi, group_label, age = data  # `labels` is a binary tensor
            spherical = reformat_data(rho, theta, phi)
            #preds = forward(model, x, spherical, positions)
            preds = model.forward(x,spherical,positions)
            
            preds = preds.cpu().numpy().flatten()
            all_preds.extend(preds)
            all_labels.extend(age.numpy().flatten())
    ...
    #print(df_all)
    corr_coef, p_value = pearsonr(all_preds, all_labels)
    # Mean squared error
    mse = np.mean((all_preds - all_labels) ** 2)
    
    #print(f"Correlation: {corr_coef:.4f}, MSE: {mse:.4f}")
    return {"Correlation": corr_coef, "MSE": mse, "p-val": p_value}, df_all
