...
_NAME_PROJ = f"{_TASK}_maerope_sex_noLoRA_conv"
_NUM_TRAIN_LAYERS = 1
...
def train(config, split_num=0):
    ...
    list_ids = {}
    for split in splits:
        split_file = os.path.join(_SPLIT_FOLDER, f"{split}_subject_list_ag_{split_num}")
    ...
    for epoch in range(max_epochs):
        ...
        val_loss = evaluate_epoch_loss(model, val_loader, criterion)
        ...
        if (epoch+1)%_INTERVAL_RECON == 0:
            ...
            metrics["epoch"]=epoch
            wandb.log(metrics)  
            csv_out = os.path.join(wandb.run.dir,f"result-tables-{_TASK}-val-sex_{epoch+1}.cvs")
            ...
            if cur_val > best_val:
                ...
                csv_out = os.path.join(wandb.run.dir,f"result-tables-{_TASK}-test-sex.cvs")
                df_all_test.to_csv(csv_out, index=False) 


    metrics, df_all = evaluate_metrics(model,val_loader)   
    wandb.log(metrics)  
    csv_out = os.path.join(wandb.run.dir,f"result-tables-{_TASK}-val-sex.cvs")
    df_all.to_csv(csv_out, index=False) 
...
def train_epoch(model, train_loader, optimizer, criterion):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        __, x, positions, rho, theta, phi, sex_label, __ = data
        spherical = reformat_data(rho, theta, phi)
        optimizer.zero_grad()
        with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16):

            out = model.forward(x,spherical,positions)
            loss = criterion(out,sex_label.to(device_backbone))
    ...
    try:
        del x, positions, rho, theta, phi, sex_label, spherical, out, loss
    except Exception:
        pass
...
def evaluate_metrics(model, metric_loader):
    ...
    with torch.no_grad():
        for i, data in enumerate(metric_loader, 0):
            ids, x, positions, rho, theta, phi, sex_label, __ = data
            spherical = reformat_data(rho, theta, phi)
            logits = model.forward(x,spherical,positions)
            ...
            all_labels.extend(sex_label.cpu().numpy().flatten())
            all_ids.extend(ids)

        try:
            del x, positions, rho, theta, phi, sex_label, spherical, logits
        except Exception:
            pass
        torch.cuda.empty_cache()
        gc.collect()

    df_all = pd.DataFrame({'ids': all_ids, 'pred': all_preds, 'gt': all_labels, 'prob': all_probs})
    print(df_all)
    ...
    return {"accuracy": acc, "precision": prec, "recall": rec, "auc": auc}, df_all
